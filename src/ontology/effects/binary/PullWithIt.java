...
public class PullWithIt extends Effect
{
    ...
    @Override
    public PullWithIt clone() throws CloneNotSupportedException
    {

        PullWithIt clonedPullWithIt = (PullWithIt)super.clone();
        clonedPullWithIt.lastGameTime = this.lastGameTime;
        clonedPullWithIt.spritesThisCycle = new ArrayList<VGDLSprite>();
        for(VGDLSprite vs: this.spritesThisCycle)
        {
            clonedPullWithIt.spritesThisCycle.add(vs.copy());
        }
        clonedPullWithIt.pixelPerfect = this.pixelPerfect;

        return clonedPullWithIt;
