...
public class WallStop extends Effect
{
    ...
    @Override
    public WallStop clone() throws CloneNotSupportedException
    {

        WallStop clonedWallStop = (WallStop)super.clone();
        clonedWallStop.friction = this.friction;
        clonedWallStop.lastGameTime = this.lastGameTime;
        clonedWallStop.spritesThisCycle = new ArrayList<VGDLSprite>();
        for(VGDLSprite vs: this.spritesThisCycle)
        {
            clonedWallStop.spritesThisCycle.add(vs.copy());
        }

        return clonedWallStop;
