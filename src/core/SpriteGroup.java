 ...
 * type.
 */
public class SpriteGroup implements Cloneable
{
    /**
    ...
    public SpriteGroup clone()
    {
        ...
        SpriteGroup clonedSpriteGroup = new SpriteGroup(this.itype);
        clonedSpriteGroup.sprites = new ConcurrentHashMap<Integer, VGDLSprite>();

        for (Map.Entry<Integer, VGDLSprite> entry : this.sprites.entrySet())
        {
            clonedSpriteGroup.sprites.put(entry.getKey(),
                    (entry.getValue()).copy());
        }
        return clonedSpriteGroup;
