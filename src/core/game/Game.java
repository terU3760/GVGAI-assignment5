...
public abstract class Game
{
    ...
    public void setWinner(Types.WINNER winner) {
        this.winner = winner;
    }
    ...
    public void setFwdModel(ForwardModel fwdModel)
    {
    ...
    public double playGame(AbstractPlayer player, int randomSeed, boolean isHuman)
    {
        ...
        {	
            ...
            /*
                Here calls the method paint of the class VGDLViewer and the method paint of the class VGDLViewer calls the method repaint of that class
                and the method repaint of that class calls the method paintComponent of that class.
                so factually here, the game draws everything and show to the user...
             */
    ...
    protected class Bucket
    {
        ...
        public Bucket clone()
        {
            ...
            for(VGDLSprite vs: this.allSprites) {
                clonedBucket.add(vs.copy());
            }
    ...
    public Game cloneExceptForwardModelAndRandomAndPathFinder()
    {
        try {

            Game clonedGame = getClass().getDeclaredConstructor().newInstance();


            clonedGame.spriteOrder = this.spriteOrder.clone();
            clonedGame.singletons = this.singletons.clone();
            clonedGame.classConst = new Content[this.classConst.length];
            int ind = 0;
            for( Content cc : this.classConst )
            {
                clonedGame.classConst[ind++] = cc.clone();
            }
            //clonedGame.classConst = SerializationUtils.clone(this.classConst);

            clonedGame.spriteGroups = new SpriteGroup[ this.spriteGroups.length ];
            ind = 0;
            for( SpriteGroup sc : this.spriteGroups )
            {
                clonedGame.spriteGroups[ind++] =  sc.clone();
            }

            clonedGame.collisionEffects = new ArrayList[this.collisionEffects.length][this.collisionEffects[0].length];
            for(int i = 0;i < this.collisionEffects.length;i++)
            {
                for(int j = 0;j < this.collisionEffects[0].length; j++)
                {

                    clonedGame.collisionEffects[i][j] = new ArrayList<Effect>();
                    for(Effect e: this.collisionEffects[i][j])
                    {
                        clonedGame.collisionEffects[i][j].add((Effect) (e.getClass().getDeclaredMethod("clone").invoke(e)));
                    }

                }
            }

            clonedGame.definedEffects = new ArrayList<Pair<Integer,Integer>>();
            for(Pair<Integer,Integer> p: this.definedEffects)
            {
                clonedGame.definedEffects.add(new Pair<Integer,Integer>(p.first,p.second));
            }

            clonedGame.eosEffects = new ArrayList[this.eosEffects.length];
            for (int i = 0;i < this.eosEffects.length;i++)
            {
                clonedGame.eosEffects[i] = new ArrayList<Effect>();
                for(Effect e : this.eosEffects[i]) {
                    clonedGame.eosEffects[i].add(e.clone());
                }
            }

            clonedGame.timeEffects = new TreeSet<TimeEffect>();
            for(TimeEffect te: this.timeEffects)
            {
                timeEffects.add(te.clone());
            }

            clonedGame.definedEOSEffects = new ArrayList<Integer>();
            clonedGame.definedEOSEffects.addAll(this.definedEOSEffects);

            clonedGame.historicEvents = new TreeSet<Event>();
            for(Event e: this.historicEvents)
            {
                clonedGame.historicEvents.add(e.copy());
            }

            clonedGame.iSubTypes = new ArrayList[this.iSubTypes.length];
            for(int i = 0;i < this.iSubTypes.length; i++) {
                clonedGame.iSubTypes[i] = new ArrayList<Integer>();
                for (Integer i2 : this.iSubTypes[i])
                {
                    clonedGame.iSubTypes[i].add(i2.intValue());
                }
            }

            clonedGame.shieldedEffects = new ArrayList[this.shieldedEffects.length];
            for(int i = 0;i < this.shieldedEffects.length;i++)
            {
                clonedGame.shieldedEffects[i] = new ArrayList<Pair<Integer,Long>>();
                for(Pair<Integer,Long> p: this.shieldedEffects[i])
                {
                    clonedGame.shieldedEffects[i].add(new Pair<Integer,Long>(p.first,p.second));
                }
            }

            clonedGame.bucketList = new Bucket[this.bucketList.length];
            for(int i = 0;i < this.bucketList.length;i++)
            {
                clonedGame.bucketList[i] = this.bucketList[i].clone();
            }

            clonedGame.charMapping = new HashMap<Character,ArrayList<String>>();
            for(Map.Entry<Character,ArrayList<String>> entry: charMapping.entrySet())
            {
                ArrayList<String> copyOfArrayListString = new ArrayList<String>();
                copyOfArrayListString.addAll((ArrayList<String>)(entry.getValue()));
                clonedGame.charMapping.put(entry.getKey().charValue(),copyOfArrayListString);
            }

            clonedGame.terminations = new ArrayList<Termination>();
            for(Termination t: this.terminations)
            {
                clonedGame.terminations.add(t.clone());
            }

            clonedGame.kill_list = new ArrayList<VGDLSprite>();
            for(VGDLSprite vs: this.kill_list)
            {
                clonedGame.kill_list.add(vs.copy());
            }

            clonedGame.resources_limits = this.resources_limits.clone();
            clonedGame.resources_colors = this.resources_colors.clone();
            clonedGame.screenSize = new Dimension(this.screenSize);
            clonedGame.size = new Dimension(this.size);
            clonedGame.is_stochastic = this.is_stochastic;
            clonedGame.num_sprites = this.num_sprites;
            clonedGame.gameTick = this.gameTick;



            clonedGame.block_size = this.block_size;
            clonedGame.avatar = (MovingAvatar)this.avatar.copy();
            clonedGame.isEnded = this.isEnded;
            clonedGame.winner = this.winner;
            clonedGame.frame_rate = this.frame_rate;


            clonedGame.score = this.score;
            clonedGame.MAX_SPRITES = this.MAX_SPRITES;
            clonedGame.random = new Random();
            clonedGame.avatarId = this.avatarId;
            clonedGame.wallId = this.wallId;
            clonedGame.disqualified = this.disqualified;
            clonedGame.nextSpriteID = this.nextSpriteID;
            clonedGame.key_handler = this.key_handler;


            // Since this is only a functional class, so I only set a pointer to it!!!
            clonedGame.pathf = this.pathf;


            clonedGame.avatarLastAction = this.avatarLastAction;
